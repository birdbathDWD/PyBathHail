...
The `input` directory is a collection of birdbath-scan example data for 3 hailstorms, pre-calculated hail radar backscatter cross sections needed for the HSD retrieval, optional ICON atmospheric model outputs for 2 hailstorms at MHP radar (only needed if it is desired to alternatively anchor the slow hail edge to the maximum rain velocity (shift_type: max_rain) instead of the default option (shift_type: min_hail_...) in `hail_config.yaml`), and the pre-determined hail analysis heights (with the `pybathspectra` toolkit); the `output` folder collects all numerical results as well as plots of some results (results both for the hail retrievals of `pybathhail` and for the radar signal postprocessing steps with `pybathspectra`).
